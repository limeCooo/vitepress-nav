...
```ts
let sym: symbol = Symbol('zhangsan')
sym = Symbol('zhangsan') // OK
sym = 'zhangsan' // Error
```
...
// 任意类型
const anyArray: any[] = ['zhangsan', 18, {}]
```
...
// 任意类型
const anyArray: Array<any> = ['zhangsan', 18, {}]
```
...
```ts
const tuple: [string, number] = ['zhangsan', 18]
